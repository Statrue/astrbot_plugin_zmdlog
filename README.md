...
| `/zmdlog 角色统计 <榜单关键词>` | 某个榜单的六星角色 DPS 分布（后面接榜单而不是角色名） |
...
- 直接输入昵称（如 `/zmdlog 某某玩家`）在榜单和副本都匹配不到时也会自动搜索公开账号。
...
- `--范围 <7d|14d|30d|all>`（也可写 `--range`，或 `7天/30天/全部`）与 `--潜能 <0|1-5|all>`（也可写 `--potential`）：只用于 `角色统计`。潜能只有零潜 / 有潜能（1–5 合并）/ 全部三档，这是 ZMDLogs 接口本身的粒度，不能只看某一层潜能。
- `角色统计` / `阵容` 后面接的是**榜单**关键词，不是角色名（想看某个角色的排名用 `--角色`）；两者只针对具体榜单；关键词命中副本或系列时会给出该副本下的榜单候选，引用回复序号即可。危机合约不提供角色统计。
