...
- 具体榜单默认展示前 10 名，可通过 `--top` 展示前 1–30 名，可用 `--角色` 只看某个主C的记录。
- 查看全部副本或某个榜单的六星角色 DPS 分布（箱线图），可按时间范围与潜能筛选。
...
| `/zmdlog <关键词> --角色 <角色名>` | 只看某个主C的排名，名次保持全榜名次 |
| `/zmdlog 角色` | 全部副本的六星角色 DPS 分布 |
| `/zmdlog 角色 <榜单关键词>` | 某个榜单的六星角色 DPS 分布（后面接榜单而不是角色名） |
| `/zmdlog 角色 [榜单关键词] --范围 <7d\|14d\|30d\|all> --潜能 <0\|1-5\|all>` | 限定统计的时间范围与角色潜能 |
...
- `--角色 <角色名>`（也可写 `--char`）：只看以该角色为主C的排名；支持全名、拼音全拼或首字母（如 `lf`），多个角色都匹配时会提示写全名。
- `--范围 <7d|14d|30d|all>`（也可写 `--range`，或 `7天/30天/全部`）与 `--潜能 <0|1-5|all>`（也可写 `--potential`）：只用于 `角色` 统计。
- `角色` / `阵容` 后面接的是**榜单**关键词，不是角色名（想看某个角色的排名用 `--角色`）；两者只针对具体榜单；关键词命中副本或系列时会给出该副本下的榜单候选，引用回复序号即可。危机合约不提供角色统计。
