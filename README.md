...
- 通过 `battleId`、战报页、分享页或排轴页链接生成战报摘要卡。卡片自上而下是：战斗贡献与本场角色、「DPS 曲线」（每秒累计折算的平均 DPS，每条线的终点等于表里那个人的 DPS）、「增益覆盖」（横向时间轴，每行一个增益，条形是它在场的时段）与「施法节奏」（竖向时间轴，每个角色一条色轨），底部是每个角色的配装概览与主要伤害来源。
...
| `/zmdlog 战报 <battleId或战报链接>` | 生成公开战报摘要卡（含 DPS 曲线、增益覆盖、施法节奏与配装概览） |
...
- 账号成绩与战报摘要分别缓存；完整战报响应保留概要、角色统计、上传时记录的阵容配装与技能统计，另外读取逐次命中里的伤害时刻（画 DPS 曲线）和按角色分组的增益（画增益覆盖）。装备词条、逐次命中与增益都按宽松规则解析，缺失或畸形的条目会被跳过而不是让整张卡失败。
