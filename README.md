...
| `/zmdlog 角色排名 [--属性 属性] [--范围 7d\|14d\|30d]` | 角色冠军榜：带每个角色的队伍在全部榜单拿下的第一名（四名角色各算一个，另计当主C 的）、前三、前十各几个，附最常见的第一名阵容和各职业位出场率；`--范围` 只算这段时间的记录 |
...
| `zmdlogs_character_standings` | 带某角色的队伍在各榜的最好名次、用时、阵容，第一名几个榜及其清单，六星再附 DPS 分布；角色名留空则是角色冠军榜；可限定一个榜只看 DPS 分布 | 角色排名页 / 冠军榜 / 角色统计页 |
