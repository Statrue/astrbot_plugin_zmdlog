...
| 工具 | 回答什么 | 附图 |
| --- | --- | --- |
| `query_endfield_board` | 某榜前几名的用时、DPS、主C、阵容、battleId，职业位出场率，出现过的阵容各几次；给角色名则只看带它的记录并统计它最常和谁同队；榜单留空则是最近的新纪录、第一名易主和各榜活跃度 | 榜单页 / 新纪录页 |
| `query_endfield_battle` | 某场战报的用时、各角色 DPS 与占比、配装养成、伤害来源、BUFF 覆盖、施放次数；给第二场则改为同一首领两场的逐项对比 | 战报卡 / 对比页 |
| `query_endfield_character` | 带某角色的队伍在各榜的最好名次、用时、阵容，第一名几个榜及其清单，六星再附 DPS 分布；角色名留空则是角色冠军榜；可限定一个榜只看 DPS 分布 | 角色排名页 / 冠军榜 / 角色统计页 |
| `query_endfield_account` | 某公开账号各首领的最好成绩，附记录数、冠军数、常用主C 和阵容；账号留空则是玩家冠军榜 | 账号页 / 玩家冠军榜 |
