...
- 向大模型提供 7 个函数工具（function calling），可以直接用自然语言问，不必记指令：榜单排名、榜单阵容统计、某角色最常同队的人、单场战报、两场对比、角色 DPS 分布、公开账号成绩。工具会把对应的长图发出来，同时把图里的事实以文字交给模型解读——数值以图为准。
...
插件注册了以下工具（工具名以 `get_endfield_` / `compare_endfield_` 开头）：

| 工具 | 回答什么 | 附图 |
| --- | --- | --- |
| `get_endfield_board_ranking` | 某榜前几名的用时、DPS、主C、阵容、battleId，以及该榜职业位出场率 | 榜单页 |
| `get_endfield_board_teams` | 某榜出现过哪些阵容、各几次 | 无 |
| `get_endfield_character_partners` | 某角色最常和谁同队、最常见的完整阵容 | 无 |
| `get_endfield_battle` | 某场战报的用时、各角色 DPS 与占比、配装养成、伤害来源、BUFF 覆盖、施放次数 | 战报卡 |
| `compare_endfield_battles` | 同一首领两场战报的各项差异 | 对比页 |
| `get_endfield_character_ranking` | 某六星干员的 DPS 分布与各榜名次 | 角色统计页 |
| `get_endfield_account` | 某公开账号各首领的最好成绩 | 账号页 |
