...
- `角色排名` 看带某个角色的队伍在每个榜单的最好名次、用时、DPS 和阵容，当主C的标出，从没出场的榜折成一行；不带角色名则是角色冠军榜，每个角色的第一名、前三、前十各几个，附最常见的第一名阵容和各职业位出场率。`玩家冠军榜` 是同一份数据按上传者数：谁上传的第一名最多，附常用主C 和阵容。数据来自后台榜单索引，页面写明截至时间。
...
- `新纪录` 看最近谁刷新了第一名、新上传了哪些记录、哪个榜最活跃。前两项来自后台索引每次重读时新旧两份排名的差异，记在本地 `record-events.json` 里（最多 1000 条、30 天）；活跃度按每条记录自己的战斗时间数，不依赖日志。
...
| `/zmdlog 角色排名 [--属性 属性] [--职业 职业] [--范围 7d\|14d\|30d]` | 角色冠军榜：带每个角色的队伍在全部榜单拿下的第一名（四名角色各算一个，另计当主C 的）、前三、前十各几个，附最常见的第一名阵容和各职业位出场率；`--范围` 只算这段时间的记录 |
| `/zmdlog 角色统计 [榜单关键词] --范围 <7d\|14d\|30d\|all> --潜能 <0\|1-5\|all>` | 限定统计的时间范围与角色潜能 |
| `/zmdlog 阵容 <榜单关键词> [--top <数量>]` | 某个榜单的职业位出场率与前 N 名常见阵容 |
| `/zmdlog 新纪录 [--范围 7d\|14d\|30d]` | 索引每次重读发现的第一名易主（谁顶掉了谁）和新上传的记录，加各榜这段时间打出的记录数；默认近 7 天 |
| `/zmdlog 玩家冠军榜 [--范围 7d\|14d\|30d]` | 各公开账号上传的第一名、前三、前十各几个，附常用主C 和常用阵容 |
...
- `--属性 <物理|灼热|寒冷|自然|电磁>`（也可写 `--element`，火/冰/雷这类叫法也认）：具体榜单只看主C 为该属性的记录；不带角色名的 `角色排名` 只看该属性的角色，回答“物理队有什么冠军”。属性来自游戏数据目录，各页阵容头像的描边颜色也按它画。
...
- `--职业 <先锋|近卫|重装|术士|突击|辅助>`（也可写 `--profession`，术师也认）：不带角色名的 `角色排名` 只看该职业的角色，并把这个职业列全：0 冠也列、谁最少直接点名、从没上过任何公开记录的也点名。
- `--范围 <7d|14d|30d|all>`（也可写 `--range`，或 `7天/30天/全部`）用于 `角色统计`、`趋势`（趋势默认 30d）和不带角色名的 `角色排名`（只算这段时间的记录，各榜名次按窗口内重新数）；`--潜能 <0|1-5|all>`（也可写 `--potential`）只用于 `角色统计`。潜能只有零潜 / 有潜能（1–5 合并）/ 全部三档，这是 ZMDLogs 接口本身的粒度，不能只看某一层潜能。
...
插件注册了 4 个工具，一个主题一个，不按功能点拆分。工具名以 `zmdlogs_` 开头，说的是数据域：`endfield_*` 是姊妹插件 zmdlore 的图鉴设定工具，`zmdlogs_*` 是这里的榜单战绩，模型看名字就分得开：

| 工具 | 回答什么 | 附图 |
| --- | --- | --- |
| `zmdlogs_board_ranking` | 某榜前几名的用时、DPS、主C、阵容、battleId，职业位出场率，出现过的阵容各几次；给角色名则只看带它的记录并统计它最常和谁同队；榜单留空则是最近的新纪录、第一名易主和各榜活跃度 | 榜单页 / 新纪录页 |
| `zmdlogs_battle_report` | 某场战报的用时、各角色 DPS 与占比、配装养成、伤害来源、BUFF 覆盖、施放次数；给第二场则改为同一首领两场的逐项对比 | 战报卡 / 对比页 |
| `zmdlogs_character_standings` | 带某角色的队伍在各榜的最好名次、用时、阵容，第一名几个榜及其清单，六星再附 DPS 分布；角色名留空则是角色冠军榜（可按属性或职业筛，按职业时该职业全部列出，0 冠和从未上榜的都点名）；可限定一个榜只看 DPS 分布 | 角色排名页 / 冠军榜 / 角色统计页 |
| `zmdlogs_account_records` | 某公开账号各首领的最好成绩，附记录数、冠军数、常用主C 和阵容；账号留空则是玩家冠军榜 | 账号页 / 玩家冠军榜 |
...
- **一次回答最多一张图，而且只在只查了一个对象时才发。** 模型回答一个问题时常连调
  两三个工具，三张长图会刷屏；只查了一个对象，就在模型答完时把那张图发出去；
  查了几个（比如比较四个角色），一张都不发——发其中任何一张都像随手挑的，
  各个对象的事实文字里都有。
