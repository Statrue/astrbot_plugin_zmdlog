...
- `角色排名` 看带某个角色的队伍在每个榜单的最好名次、用时、DPS 和阵容，当主C的标出，从没出场的榜折成一行；数据来自后台榜单索引，页面写明截至时间。
...
- 账号关注的名次从后台榜单索引里读，整份关注列表每轮零请求；索引尚未建好（刚启动的十几秒）时退回逐账号请求。只覆盖榜单列表里的榜，已退役的旧榜不再通报。
...
| `/zmdlog 角色排名 <角色名>` | 带该角色的队伍在每个榜单的最好记录：名次、用时、DPS、阵容（队伍成绩，任何星级都能查） |
...
- 后台常驻**榜单索引**：启动后把全部榜的排名读进内存（约 5MB，十几秒），之后每 20 秒重读一个榜、每分钟核对一次各榜前三名，前三名变了的榜立即重读。榜单查询、角色排名、名次通报都从索引读，几乎不再等上游；可用 `ranking_index_enabled` 关闭，`ranking_index_pace_seconds` 调节奏。
