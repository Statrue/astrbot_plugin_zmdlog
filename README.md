...
- 通过 `battleId`、战报页、分享页或排轴页链接生成战报摘要卡；卡片中部是「施法节奏」竖向时间轴（每个角色一条色轨，招式按时长占一段，普攻按连段折叠），底部附每个角色的配装概览与主要伤害来源。
...
| `/zmdlog 战报 <battleId或战报链接>` | 生成公开战报摘要卡（含施法节奏与配装概览） |
...
- 账号成绩与战报摘要分别缓存；完整战报响应保留概要、角色统计、上传时记录的阵容配装与技能统计，逐次命中的时间轴不保留。装备词条在上游是无类型字段，缺失或畸形的条目会被跳过而不是让整张卡失败。
